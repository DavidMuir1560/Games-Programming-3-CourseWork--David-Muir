...
void AlienBlastGame::InitiateSounds()      //Loads all the Game Audio from File
{
	...
	TheSoundManager->AddSound("Shoot", sounds[0]);         //Adds the Player shooting sound from File
	TheSoundManager->AddSound("Explosion", sounds[1]);     //Adds the Explision sound from File
...
void AlienBlastGame::InitiateTransforms()             // Sets the Transform for Game Objects
{
	...
	SunTransfrom.GetPos().z = TopDownCameraTransform.GetPos().z ;        //Sets the Position for the Sun
...
void AlienBlastGame::UpdateAndRender()         // Updates the Game Objects Positions and then Renders them
{
	...
	CameraManager ThirdPersonCamera(PlayerShipTransform.GetPos(), 70.0f, (float)SCREENWIDTH / (float)SCREENHEIGHT, 0.01f, 1000.0f);     //Initialises the Third Person Camera
