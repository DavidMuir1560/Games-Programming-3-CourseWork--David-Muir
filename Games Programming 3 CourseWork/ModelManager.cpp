...
ModelManager::ModelManager(VertexManager* verts, unsigned int numVerts, unsigned int* indices, unsigned int numIndices)             //Class Constructor
{
	IndexedModel model;         //The Indexed Model is Declared
	for (unsigned int i = 0; i < numVerts; i++)
	{
		model.positions.push_back(*verts[i].GetPosition());         //The Models Positions are put into an Array
		model.texCoords.push_back(*verts[i].GetTexCoord());         //The Models Texture Co-Ordinates are put into an Array
		model.normals.push_back(*verts[i].GetNormal());             //The Models Normals are put into an Array
	}
	
	for (unsigned int i = 0; i < numIndices; i++)       //For each Indice
	{
		model.indices.push_back(indices[i]);            //The Indice is put into an Array
	}

	InitMesh(model);                                    //The Mesh is Initialised
}
